...
def retrieve(query: str, tipo: str, top_k: int = 4) -> list[dict]:
    """Hybrid search restricted to one content type ('norma' or 'giurisprudenza')."""
    ...
    sql = """
        ...
        WHERE tipo = %s
        ORDER BY (
            0.4 * ts_rank(search_vector, plainto_tsquery('italian', %s))
            + 0.6 * (1 - (embedding <=> %s::vector))
        ) DESC
        LIMIT %s;
    """
    with get_conn() as conn, conn.cursor() as cur:
        cur.execute(sql, (tipo, query, query_vector, top_k))
        return cur.fetchall()
