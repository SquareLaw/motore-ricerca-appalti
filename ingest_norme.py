...
def index_article(articolo: dict):
    ...
    try:
        conn.run("""
            ...
            VALUES (:id, 'norma', :riferimento, :testo, :fonte_url, :embedding::vector)
            ON CONFLICT (id) DO UPDATE SET
                testo = EXCLUDED.testo, embedding = EXCLUDED.embedding;
        """, id=doc_id, riferimento=articolo["riferimento"], testo=articolo["testo"],
             fonte_url=NORMATTIVA_URL, embedding=embedding_to_sql(embedding))
    finally:
        conn.close()
